...
const CREATE_POLL = gql`
  mutation createPoll($proposal: String!) {
    createPoll(proposal: $proposal)
  }
`
const CreatePoll = ({ projectOwner, projectName, gitCommitHash, lips }) => {
  ...
  const {
    result: { mutate: createPoll, isBroadcasted, isMined, txHash },
  } = useWeb3Mutation(CREATE_POLL, {
    context: {
...
export async function getStaticProps() {
  ...
  const apolloSubgraphFetch = createApolloFetch({
    uri: 'https://api.thegraph.com/subgraphs/name/livepeer/livepeer-rinkeby',
  })
  const {
    data: { polls },
  } = await apolloSubgraphFetch({
  ...
  await Promise.all(
    polls.map(async () => {
      const { proposal } = await ipfs.catJSON(polls[0].proposal)
